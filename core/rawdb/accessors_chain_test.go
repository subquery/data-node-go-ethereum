...
func TestAncientStorage(t *testing.T) {
	...
	// Write and verify the header in the database
	WriteAncientBlocks(db, []*types.Block{block}, []types.Receipts{nil}, params.TestChainConfig)
...
func BenchmarkWriteAncientBlocks(b *testing.B) {
	...
	for i := 0; i < b.N; i += batchSize {
		...
		writeSize, err := WriteAncientBlocks(db, blocks, receipts, params.TestChainConfig)
		if err != nil {
			b.Fatal(err)
		}
		totalSize += writeSize
...
func TestHeadersRLPStorage(t *testing.T) {
	...
	WriteAncientBlocks(db, chain[:50], receipts[:50], params.TestChainConfig)
